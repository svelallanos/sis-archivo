...
class ReporteJobtitle extends FPDF
{
    private $jobtitle;
    private $headerData;

    public function __construct($jobtitle, $headerData)
    {
        parent::__construct();
    ...
    public function generarReporte()
    {
        ...
        $this->SetTextColor(0, 23, 82); //COLOR DEL TITU
        $this->Cell(0, 10, mb_convert_encoding('REPORTE DEL CARGO DE PERSONAL', "ISO-8859-1", "UTF-8"), 0, 1, 'C');
        $this->Ln(5);

        // Línea divisoria
        $this->SetFont('Arial', 'B', 12);
        $this->SetTextColor(2, 82, 0); //COLOR DE SUBTITULO
        $this->Cell(0, 8, mb_convert_encoding('1. DATOS DEL CARGO', "ISO-8859-1", "UTF-8"), 0, 1, 'L');
        $this->Ln(1);
        $this->SetDrawColor(0, 0, 0);
        $this->Line(10, $this->GetY(), 200, $this->GetY());
        $this->Ln(5);
        ...
        $this->agregarCampo('Nombre del Cargo:', $this->jobtitle['name']);
        $this->agregarCampo('Código del Cargo:', '#' . $this->jobtitle['id']);
        $this->agregarCampo('Descripción:', $this->jobtitle['description'], true);
        $this->agregarCampo('Estado:', $this->jobtitle['status']);
        $this->Ln(5);
        ...
        $this->SetTextColor(0, 0, 0);
        $this->agregarCampo('Fecha de registro:', $this->jobtitle['dateRegistration']);
        $this->agregarCampo('Fecha de actualización:', $this->jobtitle['dateUpdate']);
    }
    ...
    public function outputPDF($nombre = 'reporte_categoria.pdf')
    {
