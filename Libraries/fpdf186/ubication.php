...
class ReporteUbication extends FPDF
{
    private $ubication;
    private $headerData;

    public function __construct($ubication, $headerData)
    {
        parent::__construct();
    ...
    public function generarReporte()
    {
        ...
        $this->Cell(0, 10, mb_convert_encoding('REPORTE DE ALMACÉN', "ISO-8859-1", "UTF-8"), 0, 1, 'C');
        $this->Ln(5);

         // Línea divisoria
        $this->SetFont('Arial', 'B', 12);
        $this->SetTextColor(2, 82, 0); //COLOR DE SUBTITULO
        $this->Cell(0, 8, mb_convert_encoding('1. DATOS DEL ALMACÉN', "ISO-8859-1", "UTF-8"), 0, 1, 'L');
        $this->Ln(1);
        $this->SetDrawColor(0, 0, 0);
        $this->Line(10, $this->GetY(), 200, $this->GetY());
        $this->Ln(5);
        ...
        $this->agregarCampo('Nombre:', $this->ubication['name']);
        $this->agregarCampo('Código de Almacén:', '#' . $this->ubication['id']);
        $this->agregarCampo('Descripción:', $this->ubication['description']);
        $this->agregarCampo('Coordenadas:', $this->ubication['coordinates']);
        $this->agregarCampo('Url:', $this->ubication['url_google_map']);
        $this->agregarCampo('Estado:', $this->ubication['status']);
        $this->Ln(5);
        ...
        $this->SetTextColor(0, 0, 0);
        $this->agregarCampo('Fecha de registro:', $this->ubication['dateRegistration']);
        $this->agregarCampo('Fecha de actualización:', $this->ubication['dateUpdate']);
    }
    ...
    public function outputPDF($nombre = 'reporte_ubication.pdf')
    {
