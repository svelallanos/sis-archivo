...
class ReporteLoan extends FPDF
{
    private $loan;
    private $headerData;

    public function __construct($loan, $headerData)
    {
        parent::__construct();
        $this->loan = $loan;
    ...
    public function generarReporte()
    {
        ...
        $this->Cell(0, 10, mb_convert_encoding('REPORTE DE PRÉSTAMO', "ISO-8859-1", "UTF-8"), 0, 1, 'C');
        $this->SetTextColor(0, 0, 0);
        $this->Ln(5);

        // --------- DATOS DEL CLIENTE ---------
        $this->encabezadoSeccion("1. DATOS DEL CLIENTE");
        $this->agregarCampo('Nombre completo:', $this->loan['fullname']);
        $this->agregarCampo('Código del cliente:', '#' . $this->loan['customer_id']);
        $this->Ln(3);

        // --------- DETALLES DEL PRÉSTAMO ---------
        $this->encabezadoSeccion("2. DETALLES DEL PRÉSTAMO");
        $this->agregarCampo('Código del préstamo:', '#' . $this->loan['id']);

        $this->resaltarCampo('Monto:',   $this->loan['amount'], [0, 102, 0]);

        $this->agregarCampo('Fecha de inicio:', $this->loan['dateStart']);
        $this->agregarCampo('Fecha de fin:', $this->loan['dateEnd']);

        $this->resaltarCampo('Interés:', $this->loan['interest'] , [180, 0, 0]);
        $this->Ln(3);

        // --------- ESTADO Y OBSERVACIONES ---------
        $this->encabezadoSeccion("3. ESTADO Y OBSERVACIONES");
        $this->resaltarCampo('Estado:', $this->loan['status'], [0, 0, 150]);
        $this->agregarCampo('Observaciones:', $this->loan['observations']);
        $this->Ln(3);

        // --------- FECHAS DEL SISTEMA ---------
        $this->encabezadoSeccion("4. FECHAS DEL SISTEMA");
        $this->agregarCampo('Fecha de registro:', $this->loan['dateRegistration']);
        $this->agregarCampo('Fecha de actualización:', $this->loan['dateUpdate']);
    }
    ...
    public function outputPDF($nombre = 'reporte_loan.pdf')
    {
