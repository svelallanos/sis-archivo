...
function loadTable() {
  table = $("#table").DataTable({
    ...
    fnDrawCallback: function (settings) {
      // Aquí puedes agregar cualquier acción que desees realizar después de que se dibuje la tabla
      confirmationDelete();
      loadUserReport();
      loadDataUpdate();
    },
...
function loadUserReport() {
  ...
  btnReportItem.forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      reportPhotoProfile.src =
        base_url +
        "/loadfile/profile/?f=" +
        (profile == "" ? "user.png" : profile);
...
function loadDataUpdate() {
  ...
  btnUpdateItem.forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      imgUpdateProfile.src = base_url + "/loadfile/profile/?f=" + profile;
      update_txtFotoActual.value = profile;
